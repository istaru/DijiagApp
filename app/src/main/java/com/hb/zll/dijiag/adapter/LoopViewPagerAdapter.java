...
public class LoopViewPagerAdapter extends BaseLoopPagerAdapter {
    ...
    public void setList(List<BannerEntity> lists) {
        bannerEntitys.clear();
        bannerEntitys.addAll(lists);
    ...
    private void initIndicators() {
        if (mIndicators.getChildCount() != bannerEntitys.size() && bannerEntitys.size() > 1) {
            mIndicators.removeAllViews();
            Resources res = mIndicators.getResources();
            int size = res.getDimensionPixelOffset(R.dimen.dp_6);
            int margin = res.getDimensionPixelOffset(R.dimen.dp_6);
    ...
    public int getPagerCount() {
        return bannerEntitys.size();
    }

    @Override
    public BannerEntity getItem(int position) {
        return bannerEntitys.get(position);
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(parent.getContext()).inflate(R.layout.view_banner_item, parent, false);
...
//                Toast.makeText(view.getContext(), bannerInfos.get(position).getName(), Toast.LENGTH_SHORT).show();
        ...
        BannerEntity character = bannerEntitys.get(position);
